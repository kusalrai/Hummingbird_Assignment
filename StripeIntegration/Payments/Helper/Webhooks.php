...
class Webhooks
{
    ...
    // If there has been a subscription update request by the customer within the last 6 hours, we consider this webhook
    // to have arrived as part of the subscription update request.
    public function isSubscriptionUpgradeDowngrade(array $invoice)
    {
        return; // Whatever is calling this, should perform the task synchronously immediately after the subscription update

        // if (empty($invoice['billing_reason']) || empty($invoice['subscription']))
        //     return false;

        // if ($invoice['billing_reason'] != "subscription_update")
        //     return false;

        // $subscriptionModel = $this->subscriptionsHelper->loadSubscriptionModelBySubscriptionId($invoice['subscription']);
        // if ($subscriptionModel)
        // {
        //     $lastUpdated = strtotime($subscriptionModel->getLastUpdated());
        //     $sixHours = 6 * 60 * 60;
